...
/* Length of ICMP echo-specific header fields (id + seq) */
#define ICMP_ECHO_HDR_LEN (sizeof(uint16_t) + sizeof(uint16_t))


/* Total length of ICMP echo request/reply packet with maximum payload */
#define ICMP_ECHO_LEN (ICMP_COMMON_HDR_LEN + ICMP_PAYLOAD_OFFSET + FW_ICMP_ECHO_PAYLOAD_LEN)
...
static inline bool icmp_enqueue_echo_reply(fw_queue_t *icmp_queue, uintptr_t pkt_vaddr)
{
    ...
    uint16_t payload_len = icmp_total_len - ICMP_COMMON_HDR_LEN - ICMP_ECHO_HDR_LEN;
    ...
    /* Copy payload */
    uint8_t *payload_data = (uint8_t *)(pkt_vaddr + ICMP_PAYLOAD_OFFSET + ICMP_ECHO_HDR_LEN);
