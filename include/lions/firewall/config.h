...
typedef struct fw_webserver_nat_config {
    ...
    /* shared memory to store source IP and mapping timeout. If NAT component
    does not exist for an IP protocol, this region is left empty */
    region_resource_t configuration;
...
typedef struct fw_nat_config {
    ...
    fw_connection_resource_t router;
    device_region_resource_t data;
    fw_webserver_nat_config_t webserver;
...
typedef struct fw_webserver_interface_config {
    ...
    fw_webserver_nat_config_t nat[FW_MAX_FILTERS];
    uint8_t num_filters;
