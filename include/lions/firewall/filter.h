...
static const char *fw_filter_err_str[] = {"Ok.",
                                          "Out of memory error.",
                                          "Duplicate entry.",
                                          "Clashing entry.",
                                          "Invalid rule ID.",
...
static const char *fw_filter_action_str[] = {
    "No rule", "Allow", "Drop", "Reject", "Connect", "Established"};
...
typedef enum {
  FILTER_SET_DEFAULT_ARG_ACTION = 0,
  FILTER_DEFAULT_NUM_ARGS
} fw_filter_default_args_t;
...
typedef enum {
  FILTER_DELETE_ARG_RULE_ID = 0,
  FILTER_DELETE_NUM_ARGS
} fw_filter_delete_args_t;

typedef enum {
  FILTER_RET_ERR = 0,
  FILTER_RET_RULE_ID = 1
} fw_filter_ret_args_t;
 ...
 * IDs available.
 */
static fw_filter_err_t rules_reserve_id(fw_filter_state_t *state,
                                        uint16_t *rule_id) {
  ...
  for (uint16_t i = 0; i < state->rules_capacity; i++) {
    uint16_t id_to_check =
        (state->rule_id_bitmap->last_allocated_rule_id + 1 + i) %
 ...
 * otherwise.
 */
static fw_filter_err_t rules_free_id(fw_filter_state_t *state,
                                     uint16_t rule_id) {
 ...
 * @return error status.
 */
static inline fw_filter_err_t
fw_filter_add_rule(fw_filter_state_t *state, uint32_t src_ip, uint16_t src_port,
  ...
  for (uint16_t i = 0; i < state->rule_table->size; i++) {
    ...
    if ((src_port_any && !rule->src_port_any) ||
        (!src_port_any && rule->src_port_any)) {
      continue;
    }

    /* One rule applies to any dst port, one applies to a specific dst port */
    if ((dst_port_any && !rule->dst_port_any) ||
        (!dst_port_any && rule->dst_port_any)) {
      continue;
    }
 ...
 */
static inline void
fw_filter_state_init(fw_filter_state_t *state, void *rules,
                     void *rule_id_bitmap, uint16_t rules_capacity,
                     void *internal_instances,
                     region_resource_t *external_instances,
                     uint16_t instances_capacity, fw_rule_t *initial_rules,
                     uint8_t num_rules, uint8_t num_external_instances) {
  ...
  for (size_t i = 0; i < num_external_instances; i++) {
    state->external_instances_table[i] =
        (fw_instances_table_t *)external_instances[i].vaddr;
  }
  ...
  uint64_t default_mask = 1ULL
                          << (DEFAULT_ACTION_RULE_ID % RULE_ID_BITMAP_BLK_SIZE);

  /* No other rules should exist at this point */
  assert((state->rule_id_bitmap->id_bitmap[default_block_idx] & default_mask) ==
         0);
  assert(state->rule_table->size == 0);

  /* First rule must be the default rule */
  assert(num_rules >= 1);
  assert(initial_rules[DEFAULT_ACTION_IDX].src_subnet == 0 &&
         initial_rules[DEFAULT_ACTION_IDX].src_port_any);
  assert(initial_rules[DEFAULT_ACTION_IDX].dst_subnet == 0 &&
         initial_rules[DEFAULT_ACTION_IDX].dst_port_any);
  ...
  state->rule_table->rules[DEFAULT_ACTION_IDX] =
      initial_rules[DEFAULT_ACTION_IDX];
  state->rule_table->size++;

  for (uint8_t r = 1; r < num_rules; r++) {
    fw_filter_err_t err = fw_filter_add_rule(
        state, initial_rules[r].src_ip, initial_rules[r].src_port,
 ...
 * @param default_action whether connect rule was matched via filter's default
 * action.
 ...
 */
static inline fw_filter_err_t
fw_filter_add_instance(fw_filter_state_t *state, uint32_t src_ip,
                       uint16_t src_port, uint32_t dst_ip, uint16_t dst_port,
                       uint16_t rule_id, uint32_t seq) {
  ...
  for (uint16_t i = 0; i < state->internal_instances_table->size; i++) {
    ...
    if (instance->rule_id == rule_id && instance->src_ip == src_ip &&
        instance->src_port == src_port && instance->dst_ip == dst_ip &&
        instance->dst_port == dst_port) {
      return FILTER_ERR_DUPLICATE;
    }
  }

  fw_instance_t *empty_slot = state->internal_instances_table->instances +
...
fw_filter_find_action(fw_filter_state_t *state, uint32_t src_ip,
                      uint16_t src_port, uint32_t dst_ip, uint16_t dst_port,
                      uint16_t *rule_id) {
  /* First check external instances */
  for (size_t iface = 0; iface < state->num_interfaces; iface++) {
    for (uint16_t i = 0; i < state->external_instances_table[iface]->size;
         i++) {
      fw_instance_t *instance =
          state->external_instances_table[iface]->instances + i;
  ...
  for (uint16_t i = DEFAULT_ACTION_IDX + 1; i < state->rule_table->size; i++) {
    ...
    if (rule->src_subnet == match->src_subnet) {
      ...
      } else if (rule->dst_subnet >
                 match->dst_subnet) { /* destination subnet is a longer match */
        match = rule;
      }
...
static fw_filter_err_t fw_filter_remove_instances(fw_filter_state_t *state,
  ...
  while (i < state->internal_instances_table->size) {
    ...
    state->internal_instances_table->instances[i] =
        state->internal_instances_table
            ->instances[state->internal_instances_table->size - 1];
    state->internal_instances_table->size--;
  }
 ...
 */
static inline fw_filter_err_t
fw_filter_update_default_action(fw_filter_state_t *state,
  ...
  if (old_action == FILTER_ACT_CONNECT) {
    fw_filter_err_t err =
        fw_filter_remove_instances(state, DEFAULT_ACTION_RULE_ID);
    assert(err == FILTER_ERR_OKAY);
  }
...
static inline fw_filter_err_t fw_filter_remove_rule(fw_filter_state_t *state,
  ...
  generic_array_shift(state->rule_table->rules, sizeof(fw_rule_t),
                      state->rule_table->size, rule - state->rule_table->rules);
  state->rule_table->size--;
  return FILTER_ERR_OKAY;
}
