...
#define shared_elf_UIO_INIT_ADDRESS 0x300000

typedef struct shared_elf_config {
    uint32_t xres;
    uint32_t yres;
    uint32_t bpp;
} shared_elf_config_t;

shared_elf_config_t *shared_elf_config_get(void *uio_map) {
    if (uio_map == NULL) {
        printf("UIO ERROR: uio_map is NULL\n");
        return NULL;
    }
    if ((uintptr_t)uio_map % sizeof(uint32_t) != 0) {
        printf("UIO ERROR: uio_map is not aligned with shared_elf_config_t\n");
        return NULL;
    }
    return (shared_elf_config_t *)uio_map;
}

void shared_elf_config_set(void *uio_map, shared_elf_config_t config) {
    if (uio_map == NULL) {
        printf("UIO ERROR: uio_map is NULL\n");
        return;
    }
    ...
    *(shared_elf_config_t *)uio_map = config;
}

void shared_elf_base_addr(void *uio_map, uint8_t **addr) {
    if (uio_map == NULL) {
        printf("UIO ERROR: uio_map is NULL\n");
        *addr = NULL;
        return;
    ...
    *addr = (uint8_t *)uio_map + sizeof(shared_elf_config_t);
}
