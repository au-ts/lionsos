...
static size_t sock_write(const void *buf, size_t len, int fd) {
    ...
    char msg[20];
    int n = snprintf(msg, len, "sock_write: %s\n", buf);
    n = serial_enqueue_batch(&serial_tx_queue_handle, n, msg);
    if (n > 0) {
...
static long sys_socket(va_list ap) {
    ...
    int socket_handle = tcp_socket_create(NULL);
    if (socket_handle != -1) {
        socket_refcount[socket_handle]++;
        ...
        printf("sys_socket: socket created with fd %d and socket handle %d\n", fd, socket_handle);
        return fd;
    } else {
...
static long sys_bind(va_list ap) {
    ...
    printf("sys_bind: fd %d to %s:%d\n", fd, ipstr, ntohs(addr_in->sin_port), addrlen);

    return tcp_socket_bind(socket_index_of_fd(fd), addr_in);
}
...
static long sys_socket_connect(va_list ap) {
    ...
    return (long)tcp_socket_connect(socket_index_of_fd(fd), addr_in);
}
...
static long sys_listen(va_list ap) {
    ...
    printf("sys_listen: sockfd %d, backlog %d\n", sockfd, backlog);
...
static long sys_accept(va_list ap) {
    ...
    if (fd_entry != NULL) {
        *fd_entry = (fd_entry_t){
        ...
        fd_socket[fd] = socket_handle;
        printf("sys_accept: socket created with fd %d and socket handle %d\n", fd, socket_handle);
    } else {
        // TODO: cleanup socket
        return -1;
    }
...
static long sys_getsockname(va_list ap) {
    ...
    tcp_socket_getsockname(socket_index_of_fd(sockfd), addr_in);

    char ipstr[INET_ADDRSTRLEN];
    inet_ntop(AF_INET, &addr_in->sin_addr, ipstr, sizeof(ipstr));
    printf("sys_getsockname: sockfd %d has local %s:%d\n", sockfd, ipstr, ntohs(addr_in->sin_port));
...
static long sys_getpeername(va_list ap) {
    ...
    tcp_socket_getpeername(socket_index_of_fd(sockfd), addr_in);

    char ipstr[INET_ADDRSTRLEN];
    inet_ntop(AF_INET, &addr_in->sin_addr, ipstr, sizeof(ipstr));
    printf("sys_getpeername: sockfd %d has remote %s:%d\n", sockfd, ipstr, ntohs(addr_in->sin_port));
