...
#define NET_RX_QUEUE_CAPACITY_DRIV               NET_RX_QUEUE_SIZE_DRIV
...
#define NET_TX_QUEUE_CAPACITY_DRIV              NET_TX_QUEUE_SIZE_DRIV
                                       ...
                                       net_queue_info_t ret[NUM_NETWORK_CLIENTS])
{
    if (!sddf_strcmp(pd_name, NET_VIRT_RX_NAME)) {
        ret[0] = (net_queue_info_t) {
            .free = cli0_free, .active = cli0_active, .capacity = NET_RX_QUEUE_SIZE_COPY0
        };
        ret[1] = (net_queue_info_t) {
            ...
            .capacity = NET_RX_QUEUE_SIZE_COPY1
        };
    } else if (!sddf_strcmp(pd_name, NET_VIRT_TX_NAME)) {
        ret[0] = (net_queue_info_t) {
            .free = cli0_free, .active = cli0_active, .capacity = NET_TX_QUEUE_SIZE_CLI0
        };
        ret[1] = (net_queue_info_t) {
            .free = (net_queue_t *)((uintptr_t)cli0_free + 2 * NET_DATA_REGION_SIZE),
            .active = (net_queue_t *)((uintptr_t)cli0_active + 2 * NET_DATA_REGION_SIZE),
            .capacity = NET_TX_QUEUE_SIZE_CLI1
        };
    }
}
