...
{
    if (__str_match(pd_name, CLI0_NAME)) {
        net_queue_init(rx_queue, (net_queue_t *) rx_free,
                       (net_queue_t *) rx_active, NET_RX_QUEUE_SIZE_CLI0);
        net_queue_init(tx_queue, (net_queue_t *) tx_free,
                       (net_queue_t *) tx_active, NET_TX_QUEUE_SIZE_CLI0);
    } else if (__str_match(pd_name, CLI1_NAME)) {
        net_queue_init(rx_queue, (net_queue_t *) rx_free,
                       (net_queue_t *) rx_active, NET_RX_QUEUE_SIZE_CLI1);
        net_queue_init(tx_queue, (net_queue_t *) tx_free,
                       (net_queue_t *) tx_active, NET_TX_QUEUE_SIZE_CLI1);
    }
}
                                           ...
                                           uintptr_t virt_active)
{
    if (__str_match(pd_name, COPY0_NAME)) {
        net_queue_init(cli_queue, (net_queue_t *) cli_free,
                       (net_queue_t *) cli_active, NET_RX_QUEUE_SIZE_CLI0);
        net_queue_init(virt_queue, (net_queue_t *) virt_free,
                       (net_queue_t *) virt_active, NET_RX_QUEUE_SIZE_COPY0);
    } else if (__str_match(pd_name, COPY1_NAME)) {
        net_queue_init(cli_queue, (net_queue_t *) cli_free,
                       (net_queue_t *) cli_active, NET_RX_QUEUE_SIZE_CLI1);
        net_queue_init(virt_queue, (net_queue_t *) virt_free,
                       (net_queue_t *) virt_active, NET_RX_QUEUE_SIZE_COPY1);
    }
}
                                       ...
                                       uintptr_t cli_active)
{
    if (__str_match(pd_name, VIRT_RX_NAME)) {
        net_queue_init(cli_queue, (net_queue_t *) cli_free,
                       (net_queue_t *) cli_active, NET_RX_QUEUE_SIZE_COPY0);
        net_queue_init(&cli_queue[1],
                       (net_queue_t *)(cli_free + 2 * NET_DATA_REGION_SIZE),
                       (net_queue_t *)(cli_active + 2 * NET_DATA_REGION_SIZE),
                       NET_RX_QUEUE_SIZE_COPY1);
    } else if (__str_match(pd_name, VIRT_TX_NAME)) {
        net_queue_init(cli_queue, (net_queue_t *) cli_free,
                       (net_queue_t *) cli_active, NET_TX_QUEUE_SIZE_CLI0);
        net_queue_init(&cli_queue[1],
                       (net_queue_t *)(cli_free + 2 * NET_DATA_REGION_SIZE),
                       (net_queue_t *)(cli_active + 2 * NET_DATA_REGION_SIZE),
                       NET_TX_QUEUE_SIZE_CLI1);
