...
ifeq ($(strip $(FS_IMPL)),fat)
FS_IMAGE := fat.elf
FS_PD_NAME := fatfs
else ifeq ($(strip $(FS_IMPL)),ext4)
FS_IMAGE := ext4.elf
FS_PD_NAME := ext4fs
else
$(error Unsupported FS_IMPL '$(FS_IMPL)'. Supported values: fat, ext4)
endif
...
$(SYSTEM_FILE): $(METAPROGRAM) $(IMAGES) $(DTB)
	PYTHONPATH=${SDDF}/tools/meta:$$PYTHONPATH $(PYTHON) $(METAPROGRAM) --sddf $(SDDF) --board $(MICROKIT_BOARD) --dtb $(DTB) --output . --sdf $(SYSTEM_FILE) --fs $(FS_IMPL)
...
qemu_disk:
	$(SDDF)/tools/mkvirtdisk $@ 1 512 16777216 GPT $(FS_IMPL)
