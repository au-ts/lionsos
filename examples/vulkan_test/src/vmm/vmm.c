...
bool uio_init_handler(size_t vcpu_id, uintptr_t addr, size_t fsr,
...
void init(void) {
  ...
  uintptr_t kernel_pc = linux_setup_images(
      config.ram, (uintptr_t)_guest_kernel_image, kernel_size,
      (uintptr_t)_guest_dtb_image, config.dtb, dtb_size,
      (uintptr_t)_guest_initrd_image, config.initrd, initrd_size);
  if (!kernel_pc) {
  ...
  for (int i = 0; i < config.num_irqs; i++) {
    bool success = virq_register_passthrough(
        config.vcpus[0].id, config.irqs[i].irq, config.irqs[i].id);
