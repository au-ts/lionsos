...
static void tx_provide(void)
{
    ...
    for (int client = 0; client < fw_config.num_active_clients; client++)
    {
        while (!fw_queue_empty(&fw_active_clients[client]))
        {
            ...
            assert(buffer.offset % NET_BUFFER_SIZE == 0 && buffer.offset < NET_BUFFER_SIZE * fw_active_clients[client].capacity);
            ...
            if (fw_config.num_nat_configs > 0)
            {
                ...
                if (ethtype == ETH_TYPE_IP)
                {
                    ...
                    for (int j = 0; j < num_nat_modules; j++)
                    {
                        if (nat_modules[j].protocol == ip_hdr->protocol)
                        {
                            nat_result = nat_module_translate(&nat_modules[j], buffer_vaddr, (net_buff_desc_t *)&buffer, false);
                            break;
                        }
                    }
    ...
    case NAT_SET_ENABLED:
    {
        ...
        for (int i = 0; i < fw_config.num_nat_configs; i++)
        {
            if (fw_config.nat_configs[i].webserver_ch == ch)
            {
                for (int j = 0; j < num_nat_modules; j++)
                {
                    if (nat_modules[j].protocol == fw_config.nat_configs[i].protocol)
                    {
                        nat_modules[j].port_table->nat_enabled = enabled;
                        break;
                    }
                }
...
void init(void)
{
    ...
    /* Initialise NAT modules */
    num_nat_modules = 0;
    for (int i = 0; i < fw_config.num_nat_configs; i++)
    {
        fw_nat_port_table_config_t *nat_cfg = &fw_config.nat_configs[i];
        fw_nat_port_table_t *port_table = (fw_nat_port_table_t *)nat_cfg->port_table.vaddr;
        ...
        int result = nat_module_init(&nat_modules[num_nat_modules],
                                     fw_config.interface,
                                     nat_cfg->protocol,
                                     nat_cfg,
                                     port_table,
                                     fw_config.interface_ip,
                                     src_port_off,
                                     dst_port_off,
                                     check_off,
                                     check_enabled);
