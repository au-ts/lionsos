...
CONFIG_H := $(LIONSOS)/include/lions/firewall/config.h

.PHONY: force_config_h
force_config_h:

$(CONFIG_H): force_config_h $(FIREWALL_SRC_DIR)/pyfw/config_structs.py $(FIREWALL_SRC_DIR)/pyfw/cstruct.py
	@mkdir -p $(dir $@)
	PYTHONPATH=$(FIREWALL_SRC_DIR):$$PYTHONPATH $(PYTHON) $(FIREWALL_SRC_DIR)/pyfw/config_structs.py > $@

%.o: %.c $(CONFIG_H) | $(LIONS_LIBC)/include
	$(CC) $(CFLAGS) -c -o $@ $<
