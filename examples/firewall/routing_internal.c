...
__attribute__((__section__(".router_config"))) router_config_internal_t router_config;
...
net_queue_handle_t webserver_queue;
...
void process_arp_waiting()
{
    ...
    while (!arp_queue_empty_response(arp_queries)) {
        ...
        sddf_dprintf("ROUTING_INTERNAL|Processing arp waiting queue\n");
        if (waiting_packet == NULL) {
...
uint32_t find_route(uint32_t ip)
{
    ...
    // First checking if the ip address matches our IP address. Check for port 80 then forward to webserver.
    char buf[16];
    sddf_dprintf("ROUTING_INTERNAL|Finding route for this IP: %s\n", ipaddr_to_string(ip, buf, 16));
    if (ip == router_config.rx_ip) {
        return WEB_GUI_ROUTE;
    }
    // if (ip == IPV4_ADDR(192,168,33,1)) {
    //     return WEB_GUI_ROUTE;
    // }
...
void route()
{
            ...
            while (!net_queue_empty_active(&state.filter_queue[filter]) && !net_queue_empty_free(&virt_tx_queue)) {
                ...
                */
                if (pkt->ttl > 1 && pkt->type == HTONS(ETH_TYPE_IP)) {
                    ...
                    if (nextIP == WEB_GUI_ROUTE) {
                        // The ethernet + ip header will take up 34 bytes.
                        sddf_dprintf("ROUTING_INTERNAL|Was web GUI IP\n");
                        struct tcp_packet *tcp_pkt = (struct tcp_packet *)((router_config.filters[filter].data.vaddr + buffer.io_or_offset) + 34);
                        // if (tcp_pkt->dest_port == HTONS(WEB_GUI_PORT)) {
                            // @kwinter: Dont know what endinaness this will be
                            // Forward packet to the web gui here
                            sddf_dprintf("ROUTING_INTERNAL|Was web GUI port\n");
                            if (!net_queue_empty_free(&webserver_queue)) {
                                sddf_dprintf("ROUTING_INTERNAL|Forwarding packet to web gui\n");
                                net_buff_desc_t webserver_pkt;
                                int ret = net_dequeue_free(&webserver_queue, &webserver_pkt);
                                assert(!ret);
                                sddf_memcpy((router_config.webserver_conn.data.vaddr + webserver_pkt.io_or_offset), (router_config.filters[filter].data.vaddr + buffer.io_or_offset), buffer.len);
                                webserver_pkt.len = buffer.len;
                                ret = net_enqueue_active(&webserver_queue, webserver_pkt);
                                assert(!ret);
                                buffer.len = 0;
                                ret = net_enqueue_free(&state.filter_queue[filter], buffer);
                                assert(!ret);
                                // Enqueue the packet back to the filtering component.
                                microkit_deferred_notify(router_config.webserver_conn.conn.id);
                    ...
                    if (ret == -1 && !llfull(&pkt_waiting_queue)) {
                        ...
                        */
                        if (!arp_queue_full_request(arp_queries) && !check_waiting(&pkt_waiting_queue, destIP)) {
                            ...
                            if (ret != 0) {
                                sddf_dprintf("ROUTING_INTERNAL| ARP request queue was full!\n");
                            }
                        } else {
                            buffer.len = 0;
    ...
    sddf_dprintf("ROUTING_INTERNAL|Finished routing\n");
}
...
void init(void)
{
    ...
    // Setup the queues for the webserver if we have one
    net_queue_init(&webserver_queue, router_config.webserver_conn.conn.free_queue.vaddr,
    router_config.webserver_conn.conn.active_queue.vaddr, router_config.webserver_conn.conn.num_buffers);
    net_buffers_init(&webserver_queue, 0);
