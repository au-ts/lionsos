        ...
        while (!net_queue_empty_active(&rx_queue)) {
            ...
            case FILTER_ACT_CONNECT: {
                ...
                if (fw_err == FILTER_ERR_OKAY || fw_err == FILTER_ERR_DUPLICATE) {
                    LOG_FIREWALL("UDP FILTER", "on interface %u establishing connection via rule %u: (ip %s, port %u) -> "
                                 ...
                                 htons(udp_hdr->src_port), ipaddr_to_string(ip_hdr->dst_ip, ip_addr_buf1), htons(udp_hdr->dst_port));
                }

                if (fw_err == FILTER_ERR_FULL) {
                    LOG_FIREWALL_ERR("UDP FILTER", "on interface %u could not establish connection for rule %u: (ip %s, "
                                     "port %u) -> (ip %s, port %u): %s\n", filter_config.interface, rule_id, ipaddr_to_string(ip_hdr->src_ip, ip_addr_buf0),
                                     htons(udp_hdr->src_port), ipaddr_to_string(ip_hdr->dst_ip, ip_addr_buf1),
                                     htons(udp_hdr->dst_port), fw_filter_err_str[fw_err]);
                }
            }
            ...
            case FILTER_ACT_ALLOW: {
                ...
                if (action == FILTER_ACT_ALLOW || action == FILTER_ACT_CONNECT) {
                    LOG_FIREWALL("UDP FILTER", "on interface %u transmitting via rule %u: (ip %s, port %u) -> (ip %s, "
                                 "port %u)\n", filter_config.interface, rule_id, ipaddr_to_string(ip_hdr->src_ip, ip_addr_buf0),
                                 htons(udp_hdr->src_port), ipaddr_to_string(ip_hdr->dst_ip, ip_addr_buf1), htons(udp_hdr->dst_port));
                } else if (action == FILTER_ACT_ESTABLISHED) {
                    LOG_FIREWALL("UDP FILTER", "on interface %u transmitting via external rule %u: (ip %s, port %u) "
                                 "-> (ip %s, port %u)\n", filter_config.interface, rule_id, ipaddr_to_string(ip_hdr->src_ip, ip_addr_buf0),
                                 htons(udp_hdr->src_port), ipaddr_to_string(ip_hdr->dst_ip, ip_addr_buf1), htons(udp_hdr->dst_port));
                }
                break;
            ...
            case FILTER_ACT_REJECT: {
                ...
                LOG_FIREWALL("UDP FILTER", "on interface %u, filter rejecting via rule %u: (ip %s, port %u) -> "
            ...
            default: {
                ...
                LOG_FIREWALL("UDP FILTER", "on interface %u dropping via rule %u: (ip %s, port %u) -> (ip %s, port %u)\n",
                             filter_config.interface, rule_id, ipaddr_to_string(ip_hdr->src_ip, ip_addr_buf0),
                             htons(udp_hdr->src_port), ipaddr_to_string(ip_hdr->dst_ip, ip_addr_buf1), htons(udp_hdr->dst_port));
                break;
            }
...
microkit_msginfo protected(microkit_channel ch, microkit_msginfo msginfo)
{
    ...
    case FILTER_SET_DEFAULT_ACTION: {
        ...
        LOG_FIREWALL("UDP FILTER", "on interface %u changing default action from %u to %u\n",
    ...
    case FILTER_ADD_RULE: {
        ...
        LOG_FIREWALL("UDP FILTER", "on interface %u create rule %u: (ip %s, mask %u, port %u, any_port %u) - (%s) -> "
    ...
    case FILTER_DEL_RULE: {
        ...
        LOG_FIREWALL("UDP FILTER", "on interface %u remove rule id %u: %s\n", filter_config.interface, rule_id,
    ...
    default:
        LOG_FIREWALL_ERR("UDP FILTER", "on interface %u, unknown request %lu on channel %u\n", filter_config.interface,
...
void notified(microkit_channel ch)
{
    if (ch == net_config.rx.id) {
        ...
        LOG_FIREWALL_ERR("UDP FILTER", "on interface %u, received notification on unknown channel: %d!\n",
