...
static void filter(void)
{
    ...
    while (reprocess) {
        while (!net_queue_empty_active(&rx_queue)) {
            ...
            // generic. If the connection is already being tracked, it will
            // return a pointer to the instance entry
            ...
            case FILTER_ACT_ESTABLISHED: {
                ...
                /* Extract TCP tracking state */
                fw_tcp_packet_data_t *internal_tracking = NULL;
                fw_tcp_packet_data_t *external_tracking = NULL;
                fw_tcp_conn_state_t conn_state = TCP_NONE;
                fw_filter_err_t fw_err = fw_tcp_extract_state(&filter_state, instance, &internal_tracking, &external_tracking, &conn_state);
                ...
                case TCP_NONE: {
                    ...
                    if (fw_err == FILTER_ERR_OKAY && FW_DEBUG_OUTPUT) {
                        sddf_printf(
                ...
                case TCP_SYN_SENT: {
                    ...
                    if (!((syn && !ack && !fin) && seq == internal_tracking->seq)) {
                        forward_packet = false;
                    }
                    break;
                }
                case TCP_SYN_SEEN: {
                    ...
                    if (!((syn && ack && !fin) && ack_seq == external_tracking->seq + 1)) {
                        forward_packet = false;
                        break;
                    }

                    internal_tracking->flags = fw_tcp_flags_to_bits(syn, ack, fin, false);
                    internal_tracking->seq = seq;
                    break;
                }
                case TCP_SYNACK_SENT: {

                    /* Only syn-ack retry packets permitted. */
                    if (!((syn && ack && !fin) && ack_seq == external_tracking->seq + 1 && seq == internal_tracking->seq)) {
                        forward_packet = false;
                    }
                    break;
                }
                case TCP_SYNACK_SEEN: {
                    ...
                    if (!((!syn && ack && !fin) && ack_seq == external_tracking->seq + 1 && seq == internal_tracking->seq + 1)) {
                        forward_packet = false;
                        break;
                    }
                ...
                case TCP_FIN_SENT: {
                    ...
                    if (!(!syn && ack && ((fin && seq == internal_tracking->seq) || (!fin && seq == internal_tracking->seq + 1)))) {
                        forward_packet = false;
                    }
                    break;
                }
                case TCP_FIN_SEEN: {
                    ...
                    if (fin && (ack_seq != external_tracking->seq + 1)) {
                        forward_packet = false;
                        break;
                    }
                ...
                case TCP_FINACK_SENT: {
                    ...
                    if (!((!syn && fin && ack) && seq == internal_tracking->seq && ack_seq == external_tracking->seq + 1)) {
                        forward_packet = false;
                    }
                    break;
                }
                case TCP_FINACK_SEEN: {
                    ...
                    if (!((!syn && !fin && ack) && seq == internal_tracking->seq + 1 && ack_seq == external_tracking->seq + 1)) {
                        forward_packet = false;
                        break;
                    }
