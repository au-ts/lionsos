...
static void translate(void)
{
    ...
    while (!fw_queue_empty(&filter_queue)) {
        ...
        if (nat_interface_config.snat) {
            ...
            if (port_table->size < nat_interface_config.ports_capacity) {
                port_table->mappings[port_table->size].src_port = udp_hdr->src_port;
                port_table->mappings[port_table->size].dst_port = udp_hdr->dst_port;
                port_table->mappings[port_table->size].src_ip = ip_hdr->src_ip;
                port_table->mappings[port_table->size].dst_ip = ip_hdr->dst_ip;

                ip_hdr->src_ip = nat_interface_config.snat;
                ip_hdr->check = 0;

                udp_hdr->src_port = htons(nat_interface_config.base_port + port_table->size);
                udp_hdr->check = 0;

                if (FW_DEBUG_OUTPUT) {
                    sddf_dprintf("%sUDP NAT LOG: translated to %s:%u\n",
                                 fw_frmt_str[nat_config.interface],
                                 ipaddr_to_string(nat_interface_config.snat, ip_addr_buf1),
