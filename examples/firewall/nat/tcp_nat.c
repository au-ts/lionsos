...
static void log_packet(ipv4_hdr_t *ip_hdr, tcp_hdr_t *tcp_hdr)
{
    if (FW_DEBUG_OUTPUT) {
        ...
        sddf_printf("%sTCP NAT LOG: dst = %s:%u\n", fw_frmt_str[nat_config.interface], ipaddr_to_string(ip_hdr->dst_ip,
                                                                                                        ip_addr_buf0), htons(tcp_hdr->dst_port));
    }
}
...
static void translate(void)
{
    ...
    while (!fw_queue_empty(&filter_queue)) {
        ...
        if (dst_mapping) {
            if (FW_DEBUG_OUTPUT) {
                sddf_printf("%sTCP NAT LOG: returning traffic detected\n", fw_frmt_str[nat_config.interface]);
            }
            tcp_hdr->dst_port = dst_mapping->src_port;
        ...
        if (nat_interface_config.snat) {
            ...
            if (ephemeral_port) {
                ...
                if (FW_DEBUG_OUTPUT) {
                    sddf_printf("%sTCP NAT LOG: translated to %s:%u\n",
                                fw_frmt_str[nat_config.interface],
...
void init(void)
{
    ...
    if (FW_DEBUG_OUTPUT) {
        sddf_printf("%sTCP NAT LOG: base port: %u\ncapacity: %u\nsnat: %u\n", fw_frmt_str[nat_config.interface],
