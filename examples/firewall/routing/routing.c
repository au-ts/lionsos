...
fw_queue_t tx_active[FW_MAX_INTERFACES][FW_MAX_INTERFACES]; /* Queues to transmit packets out any network interface */
...
static bool tx_net[FW_MAX_INTERFACES][FW_MAX_INTERFACES]; /* Packet has been transmitted to the
...
static int enqueue_icmp_unreachable(net_buff_desc_t buffer, fw_interface_id_t src_interface)
{
    icmp_req_t req = { 0 };
...
static void transmit_packet(net_buff_desc_t buffer, uint8_t *mac_addr, fw_interface_id_t src_interface,
                            fw_interface_id_t out_interface)
{
    uintptr_t pkt_vaddr = data_vaddr[src_interface] + buffer.io_or_offset;
    ...
    int err = fw_enqueue(&tx_active[src_interface][out_interface], &buffer);
    assert(!err);
    tx_net[src_interface][out_interface] = true;
}
...
static void process_arp_waiting(fw_interface_id_t out_interface)
{
    while (!fw_queue_empty(&arp_resp_queue[out_interface])) {
        ...
        if (FW_DEBUG_OUTPUT) {
            sddf_printf("Router dequeuing response for ip %s on interface %u and MAC[0] "
                        "= %x, MAC[5] = %x\n",
        ...
        if (response.state == ARP_STATE_UNREACHABLE) {
            ...
            for (uint16_t i = 0; i < root->num_children + 1; i++) {
                err = enqueue_icmp_unreachable(node->buffer, node->src_interface);
                if (FW_DEBUG_OUTPUT && err) {
                    sddf_printf("ROUTING LOG: Could not enqueue ICMP unreachable!\n");
                }
                err = fw_enqueue(&rx_free[node->src_interface], &node->buffer);
                returned[node->src_interface] = true;
                assert(!err);
            ...
            pkt_waiting_node_t *node = root;
            for (uint16_t i = 0; i < root->num_children + 1; i++) {
                transmit_packet(node->buffer, response.mac_addr, node->src_interface, node->dst_interface);
                node = pkts_waiting_next_child(&pkt_waiting_queue, node);
            }
        }
...
{
    for (fw_interface_id_t interface = 0; interface < router_config.num_interfaces; interface++) {
        for (int filter = 0; filter < router_config.interfaces[interface].num_filters; filter++) {
            while (!fw_queue_empty(&fw_filters[interface][filter])) {
                ...
                uint32_t next_hop;
                fw_interface_id_t out_interface;
                fw_routing_err_t fw_err = fw_routing_find_route(routing_table, ip_hdr->dst_ip, &next_hop,
                                                                &out_interface, 0);
                assert(fw_err == ROUTING_ERR_OKAY);

                if (FW_DEBUG_OUTPUT && out_interface != FW_ROUTING_INTERFACE_NONE) {
                ...
                if (out_interface == FW_ROUTING_INTERFACE_NONE) {
                    if (FW_DEBUG_OUTPUT) {
                ...
                if (out_interface == FW_ROUTING_INTERFACE_SELF) {
                    if (FW_DEBUG_OUTPUT) {
                        sddf_printf("Router transmitted packet to webserver\n");
                    }
                ...
                if ((arp != NULL && arp->state == ARP_STATE_UNREACHABLE)
                    ...
                    if (arp != NULL && arp->state == ARP_STATE_UNREACHABLE) {
                        ...
                        sddf_dprintf("ROUTING LOG: Waiting packet or ARP request queue "
                ...
                if (arp == NULL || arp->state == ARP_STATE_PENDING) {
                    ...
                    if (root) {
                        ...
                        fw_err = pkt_waiting_push_child(&pkt_waiting_queue, root, buffer, interface, out_interface);
                        assert(fw_err == ROUTING_ERR_OKAY);
                    } else {
                        ...
                        assert(!err);
                        fw_err = pkt_waiting_push(&pkt_waiting_queue, next_hop, buffer, interface, out_interface);
                        assert(fw_err == ROUTING_ERR_OKAY);
...
void init(void)
{
    ...
    for (fw_interface_id_t interface = 0; interface < router_config.num_interfaces; interface++) {
        ...
        /* For a given interface the router must have N tx queues */
        for (fw_interface_id_t tx_interface = 0; tx_interface < router_config.num_interfaces; tx_interface++) {
            /* Set up virt tx firewall queue */
            fw_queue_init(&tx_active[interface][tx_interface], iface->tx_active[tx_interface].queue.vaddr,
                          sizeof(net_buff_desc_t), iface->tx_active[tx_interface].capacity);
        }
    ...
    for (fw_interface_id_t i = 0; i < router_config.num_interfaces; i++) {
        if (i == router_config.webserver_interface) {
            fw_routing_table_add_route(routing_table, FW_ROUTING_INTERFACE_SELF, router_config.interfaces[i].ip, 32,
                                       router_config.interfaces[i].ip);
        }
        fw_routing_table_add_route(routing_table, i, router_config.interfaces[i].ip, router_config.interfaces[i].subnet,
    ...
    fw_queue_init(&webserver, router_config.webserver_rx.queue.vaddr, sizeof(net_buff_desc_t),
...
void notified(microkit_channel ch)
{
    ...
    for (fw_interface_id_t interface = 0; interface < router_config.num_interfaces; interface++) {
        ...
        if (returned[interface]) {
            ...
            microkit_deferred_notify(router_config.interfaces[interface].rx_free.ch);
        }

        for (fw_interface_id_t tx_interface = 0; tx_interface < router_config.num_interfaces; tx_interface++) {
            if (tx_net[interface][tx_interface]) {
                tx_net[interface][tx_interface] = false;
                microkit_deferred_notify(router_config.interfaces[interface].tx_active[tx_interface].ch);
            }
        }
