...
fw_queue_t tx_active[FW_MAX_INTERFACES];                    /* Queues to transmit packets out any network interface */
...
void notified(microkit_channel ch)
{
    ...
    for (fw_interface_id_t interface = 0; interface < router_config.num_interfaces; interface++) {
        ...
        if (tx_net[interface]) {
            ...
            microkit_deferred_notify(router_config.interfaces[interface].tx_active.ch);
        }
