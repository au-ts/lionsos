...
static void generate_icmp(void)
{
    ...
    while (!fw_queue_empty(&icmp_queue)) {
        icmp_req_t req = { 0 };
        int err = fw_dequeue(&icmp_queue, &req);
        assert(!err);

        uint8_t out_int = req.out_interface;
        assert(out_int < icmp_config.num_interfaces);
        ...
        case ICMP_DEST_UNREACHABLE:
            ...
            // TODO: Should this be less if the source packet did not contain 8
            // bytes of data?
            ...
            /* Unused must be set to 0, as well as optional fields we are not
            currently using */
            icmp_dest->unused = 0;
            icmp_dest->len = 0;
        ...
        /* Set checksum to 0 and leave calculation to hardware. If this is not
        supported, calculate IP and ICMP checksums here */
        icmp_hdr->check = 0;
        ip_hdr->check = 0;
...
void init(void)
{
    ...
    fw_queue_init(&icmp_queue, icmp_config.router.queue.vaddr, sizeof(icmp_req_t), icmp_config.router.capacity);
