        ...
        while (!fw_queue_empty(&icmp_queue[out_int]) && !net_queue_empty_free(&net_queue[out_int])) {
            ...
            switch (req.type) {
                case ICMP_DEST_UNREACHABLE:
                    ...
                    ip_hdr->tot_len = IPV4_HDR_LEN_MIN + ICMP_DEST_LEN;
                    ...
                    uint16_t to_copy = MIN(FW_ICMP_SRC_DATA_LEN, ip_hdr->tot_len - IPV4_HDR_LEN_MIN);
            ...
            icmp_hdr->check = fw_internet_checksum(&icmp_hdr, ip_hdr->tot_len - IPV4_HDR_LEN_MIN);
            /* IP checksum is calculated only over IP header */
            ip_hdr->check = fw_internet_checksum(&ip_hdr, IPV4_HDR_LEN_MIN);
            #endif

            buffer.len = ip_hdr->tot_len + ETH_HDR_LEN;
