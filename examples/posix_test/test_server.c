...
static bool test_integration_blocking_echo() {
    ...
    EXPECT_OK(ntohs(local_addr.sin_port) == INT_PORT_BLOCKING);
...
/* Integration test: non-blocking echo server */
static bool test_integration_nonblocking_echo() {
    ...
    /* Non-blocking accept with retry */
    ...
    // wait for client to say they've finished
...
static bool test_integration_connect_refused_sync() {
...
void run_tests(void) {
    ...
    /* Integration tests */
    if (!test_integration_connect_refused_sync()) {
        return;
    }
