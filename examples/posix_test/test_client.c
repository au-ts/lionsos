...
#define INT_PORT_REFUSED 5559
#define INT_PORT_BLOCKING 5560
#define INT_PORT_NONBLOCK 5561
...
static volatile bool dhcp_ready = false;
...
static bool test_integration_connect_refused() {
    int sock = -1;
    bool result = false;
    struct sockaddr_in addr;
    ...
    memset(&addr, 0, sizeof(addr));
    addr.sin_family = AF_INET;
    addr.sin_port = htons(INT_PORT_REFUSED);
    addr.sin_addr.s_addr = inet_addr(HOST_IP);
...
/* Integration test: blocking echo client */

static bool test_integration_blocking_echo() {
    int sock = -1;
    bool result = false;
    ...
    memset(&addr, 0, sizeof(addr));
    addr.sin_family = AF_INET;
    addr.sin_port = htons(INT_PORT_BLOCKING);
    addr.sin_addr.s_addr = inet_addr(HOST_IP);
    ...
    EXPECT_OK(peer_addr.sin_port == htons(INT_PORT_BLOCKING));
...
/* Integration test: non-blocking echo client */
static bool test_integration_nonblocking_echo() {
    int sock = -1;
    bool result = false;
    struct sockaddr_in addr;
    char buf[64];
    ...
    memset(&addr, 0, sizeof(addr));
    addr.sin_family = AF_INET;
    addr.sin_port = htons(INT_PORT_NONBLOCK);
    addr.sin_addr.s_addr = inet_addr(HOST_IP);
    ...
    // let the server know we've got it
    microkit_notify(SERVER_NTFN_CH);
...
void run_tests(void) {
    ...
    printf("POSIX_TEST|client|INFO|Unit tests passed\n");

    /* Integration tests */
    if (!test_integration_connect_refused()) {
        return;
    }
    if (!test_integration_blocking_echo()) {
        return;
    }
    if (!test_integration_nonblocking_echo()) {
        return;
    }
