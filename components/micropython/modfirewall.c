...
static inline fw_routing_table_t *get_routing_table(void)
{
    return (fw_routing_table_t *)fw_config.router.routing_table.vaddr;
}
...
static mp_obj_t interface_get_mac(mp_obj_t interface_idx_in)
{
    ...
    if (interface_idx >= FW_MAX_INTERFACES) {
...
static mp_obj_t interface_get_name(mp_obj_t interface_idx_in)
{
    ...
    const char *name = (const char *)fw_config.interfaces[interface_idx].name;
    size_t len = strnlen(name, FW_INTERFACE_NAME_LEN);
    return mp_obj_new_str(name, len);
}
...
/* Count the number of routes in the unified routing table */
static mp_obj_t route_count(void)
{
    return mp_obj_new_int_from_uint(get_routing_table()->size);
}
...
static mp_obj_t route_get_nth(mp_obj_t route_idx_in)
{
    ...
    if (route_idx >= get_routing_table()->size) {
        sddf_dprintf("WEBSERVER|LOG: %s\n", fw_os_err_str[OS_ERR_INVALID_ROUTE_NUM]);
        mp_raise_OSError(OS_ERR_INVALID_ROUTE_NUM);
        return mp_const_none;
    }

    fw_routing_entry_t *entry = (fw_routing_entry_t *)(get_routing_table()->entries + route_idx);
...
static mp_obj_t rule_get_nth(mp_obj_t interface_idx_in, mp_obj_t protocol_in, mp_obj_t rule_idx_in)
{
    ...
    if (rule_idx == DEFAULT_ACTION_IDX
        || rule_idx >= webserver_state[interface_idx].filter_states[protocol_match].rule_table->size) {
        sddf_dprintf("WEBSERVER|LOG: %s\n", fw_os_err_str[OS_ERR_INVALID_RULE_NUM]);
        mp_raise_OSError(OS_ERR_INVALID_RULE_NUM);
        return mp_const_none;
    }

    fw_rule_t *rule = (fw_rule_t *)(webserver_state[interface_idx].filter_states[protocol_match].rule_table->rules
                                    + rule_idx);
    mp_obj_t tuple[10];
